...
public class EventUtil {
	public static Map<String, String[]> getRequestHeadersInArray(Map<String, List<String>> requestHeaders) {
		...
		requestHeaders.entrySet()
				.forEach(entry -> requestHeadersArray.put(entry.getKey(), entry.getValue().toArray(new String[0])));
		return requestHeadersArray;
	}

	public static void handleFixedBatchIdRequest(Request request) {
		String courseIdKey = request.getRequest().containsKey(JsonKey.COURSE_ID) ? JsonKey.COURSE_ID
				: (request.getRequest().containsKey(JsonKey.ENROLLABLE_ITEM_ID) ? JsonKey.ENROLLABLE_ITEM_ID
		...
		// Till we add a type, we will use fixed batch identifier to prefix to course to
		// get the batchId
