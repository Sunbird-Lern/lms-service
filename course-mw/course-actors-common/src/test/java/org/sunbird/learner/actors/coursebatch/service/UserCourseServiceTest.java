...
public class UserCourseServiceTest {
  ...
  public static void setup() {
    ...
    when(EsClientFactory.getInstance()).thenReturn(esUtil);
  }
  ...
  public void getPrimaryKeyMapTest() {
    ...
    Assert.assertEquals(UserCoursesService.getPrimaryKey(map),
        OneWayHashing.encryptVal(
            JsonKey.USER_ID
                + JsonKey.PRIMARY_KEY_DELIMETER
                + JsonKey.COURSE_ID
  ...
  public void getBatchSizeDefaultTest() {
    ...
    Assert.assertEquals(new Integer(10), userCoursesService.getBatchSize(JsonKey.CASSANDRA_WRITE_BATCH_SIZE));
  }
  ...
  public void getActiveUserCourseTestSuccess() {
    ...
    when(esUtil.search(Mockito.any(), Mockito.anyString(), Mockito.any())).thenReturn(promise.future());
