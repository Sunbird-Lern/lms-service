...
class BbbApi extends Meet {
  ...
  @throws[BBBException]
  def getMeetingInfo(meetingID: String): Meeting = try {
    ...
    Meeting(returnCode = response.get(ProviderConstants.RESPONSE_RETURN_CODE).asInstanceOf[String], name = response.get(ProviderConstants.RESPONSE_MEETING_NAME).asInstanceOf[String])
  } catch {
    case e: BBBException => e.getMessageKey match {
      case BBBException.MESSAGE_KEY_NOT_FOUND =>
        println(s"Meeting with id : {$meetingID} not found")
        null
      case _ => throw e
    }
  }
