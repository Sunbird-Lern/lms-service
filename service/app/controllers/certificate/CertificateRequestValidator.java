...
public class CertificateRequestValidator extends BaseRequestValidator {
  ...
  public void validateAddCertificateRequest(Request certRequestDto) {
    Map<String, Object> batch = (Map<String, Object>) certRequestDto.getRequest().get(JsonKey.BATCH);
    validateParam(
    ...
    Map<String, Object> template = (Map<String, Object>)batch.get(CourseJsonKey.TEMPLATE);
      ...
      validateParam(
              (String) template.get(JsonKey.NAME),
              ResponseCode.mandatoryParamsMissing,
              JsonKey.NAME);
      validateTemplateCriteria(template);
      if (template.containsKey(CourseJsonKey.ISSUER) && !(template.get(CourseJsonKey.ISSUER) instanceof Map)) {
        throw new ProjectCommonException(
                ResponseCode.dataTypeError.getErrorCode(),
                MessageFormat.format(
      ...
      if (template.containsKey(CourseJsonKey.SIGNATORY_LIST) && !(template.get(CourseJsonKey.SIGNATORY_LIST) instanceof List)) {
        throw new ProjectCommonException(
                ResponseCode.dataTypeError.getErrorCode(),
                MessageFormat.format(
  ...
  public void validateTemplateCriteria(Map<String,Object> template){
    if(!template.containsKey(JsonKey.CRITERIA))
    {
      ...
      if(!(criteria.containsKey(CourseJsonKey.ENROLLMENT)) && !(criteria.containsKey(JsonKey.ASSESSMENT))){
        throw new ProjectCommonException(
                ResponseCode.mandatoryParamsMissing.getErrorCode(),
                MessageFormat.format(
      ...
      if(criteria.containsKey(CourseJsonKey.ENROLLMENT) && !(criteria.get(CourseJsonKey.ENROLLMENT) instanceof Map)){
        throw new ProjectCommonException(
                ResponseCode.dataTypeError.getErrorCode(),
                MessageFormat.format(
      ...
      if(criteria.containsKey(CourseJsonKey.ENROLLMENT) && !((Map<String, Object>)criteria.get(CourseJsonKey.ENROLLMENT)).containsKey(JsonKey.STATUS)){
        throw new ProjectCommonException(
                ResponseCode.mandatoryParamsMissing.getErrorCode(),
                MessageFormat.format(
      ...
      if(criteria.containsKey(CourseJsonKey.ENROLLMENT) && ((Map<String, Object>)criteria.get(CourseJsonKey.ENROLLMENT)).size()>1){
        throw new ProjectCommonException(
      ...
      if(criteria.containsKey(JsonKey.ASSESSMENT)) {
        ...
        if(!(score.containsKey("ge") || score.containsKey("le") || score.containsKey("eq"))){
          throw new ProjectCommonException(
                  ResponseCode.invalidPropertyError.getErrorCode(),
                  MessageFormat.format(
                          ResponseCode.invalidPropertyError.getErrorMessage(), "for Score"),
                  ResponseCode.CLIENT_ERROR.getResponseCode());
        }
